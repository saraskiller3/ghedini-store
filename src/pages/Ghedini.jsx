...
import PrivacyPolicyLT from "./PrivacyPolicyLT";
import TermsConditionsLT from "./TermsConditionsLT";
import emailjs from "@emailjs/browser";
import Navbar from "../components/Navbar";
import MulceriaiPage from "./MulceriaiPage";
import MulcersPage from "./MulcersPage";
import ZemesGraztai from "./ZemesGraztai";
import Augers from "./Augers";
import LogGrabs from "./LogGrabs";
import MedziuGriebtuvai from "./MedziuGriebtuvai";
import SubmersiblePumps from "./SubmersiblePumps";
import PanardinamiSiurbliai from "./PanardinamiSiurbliai";
import Vibroplates from "./Vibroplates";
import Vibroplokstes from "./Vibroplokstes";
import Ripper from "./Ripper";
import Riperiai from "./Riperiai";
import HedgeTrimmer from "./HedgeTrimmer";
import GyvatvoriuKirpimas from "./GyvatvoriuKirpimas";
import SienavimoKausas from "./SienavimoKausas";
...
const SEO_ROUTES = {
    "/ghedini/earth-augers": "/ghedini/zemes-graztai",
    "/ghedini/zemes-graztai": "/ghedini/earth-augers",

    "/ghedini/mulchers": "/ghedini/mulceriai",
    "/ghedini/mulceriai": "/ghedini/mulchers",

    "/ghedini/log-grabs": "/ghedini/medziu-griebtuvai",
    "/ghedini/medziu-griebtuvai": "/ghedini/log-grabs",

    "/ghedini/submersible-pumps": "/ghedini/panardinami-siurbliai",
    "/ghedini/panardinami-siurbliai": "/ghedini/submersible-pumps",

    "/ghedini/vibroplates": "/ghedini/vibroplokstes",
    "/ghedini/vibroplokstes": "/ghedini/vibroplates",

    "/ghedini/ripper": "/ghedini/riperis",
    "/ghedini/riperis": "/ghedini/ripper",

    "/ghedini/hedge-cutter": "/ghedini/gyvatvoriu-kirpimo-irenginys",
    "/ghedini/gyvatvoriu-kirpimo-irenginys": "/ghedini/hedge-cutter",

    "/ghedini/mowing-bucket": "/ghedini/sienavimo-kausas",
    "/ghedini/sienavimo-kausas": "/ghedini/mowing-bucket",

    "/ghedini/pile-driver": "/ghedini/poliu-kaltuvas",
    "/ghedini/poliu-kaltuvas": "/ghedini/pile-driver",
};
        ...
        <main className="min-h-[60vh]">
            <section className="border-b border-neutral-800 bg-[radial-gradient(60%_80%_at_50%_-10%,rgba(253,224,71,0.12)_0%,transparent_70%)]">
                <div className="mx-auto max-w-7xl px-4 py-10">
                    <button onClick={() => navigate(-1)} className="text-sm text-neutral-400 hover:text-white">← {tOld("Back", "Atgal")}</button>
            ...
            <section className="mx-auto max-w-7xl px-4 py-10">
                {items.length === 0 ? (
                    <div className="text-neutral-400">{tOld("No items yet in this category.", "Šioje kategorijoje dar nėra įrašų.")}</div>
                ) : (
                ...
                <div className="mt-10 flex flex-wrap gap-3">
                    <button onClick={() =>
                        scrollTo("contact")} className="rounded-2xl border border-neutral-700 px-4 py-2 text-white hover:bg-neutral-900">{tOld("General inquiry", "Bendra užklausa")}</button>
                    <button onClick={() =>
                        scrollTo("catalog")} className="rounded-2xl bg-yellow-500 text-black px-4 py-2 font-medium hover:bg-yellow-400">{tOld("Back to all products", "Grįžti į visus produktus")}</button>
                </div>
            </section>
        </main>
    );
...
function ProductPage({ lang, setEnquire }) {
    ...
    const t = (lt, en) => (lang === "lt" ? lt : en);
    const tOld = (en, lt) => t(lt, en);
    ...
    if (!p) {
        return (
            <main className="mx-auto max-w-7xl px-4 py-16">
                <button onClick={() => navigate(-1)} className="text-sm text-neutral-400 hover:text-white">← {tOld("Back", "Atgal")}</button>
                <h1 className="mt-4 text-2xl font-bold">{tOld("Product not found", "Produktas nerastas")}</h1>
            </main>
        );
    }
    ...
    return (
        <main className="min-h-[60vh]">
            ...
            <section className="border-b border-neutral-800 bg-[radial-gradient(60%_80%_at_50%_-10%,rgba(253,224,71,0.12)_0%,transparent_70%)]">
                <div className="mx-auto max-w-7xl px-4 py-10">
                    <button onClick={goBack} className="text-sm text-neutral-400 hover:text-white">← {tOld("Back", "Atgal")}</button>
            ...
            <section className="mx-auto max-w-7xl px-4 py-10 grid gap-10 md:grid-cols-2 md:items-start">
                ...
                <div className="grid gap-4">
                    ...
                    <div className="flex flex-wrap gap-3">
                        ...
                        >
                            {tOld("Inquire about this product", "Užklausti dėl šio produkto")}
                        </button>
                        <button
                            onClick={() => document.getElementById("contact")?.scrollIntoView({ behavior: "smooth" })}
                            className="rounded-2xl border border-neutral-700 px-4 py-2 hover:bg-neutral-900"
                        >
                            {tOld("General inquiry", "Bendra užklausa")}
                        </button>
                    </div>
                </div>
            </section>
...
export default function Ghedini({ lang, handleLangChange }) {
        ...
        <div className="min-h-screen bg-black text-white">
                    ...
                    <div>
                                            <h1 className="text-3xl md:text-5xl font-extrabold tracking-tight flex items-center gap-4">
                                                <img
                                                    ...
                                                    draggable="false"
                                                />
                                                    {tOld("Hydraulic Attachments", "Hidrauliniai priedai")}</h1>
                        <p className="mt-4 text-neutral-300 md:text-lg">{tOld("Showcase of attachments for excavators and loaders in the Baltics.", "Priedai ekskavatoriams ir krautuvams Baltijos šalyse.")}</p>
                        <div className="mt-6 flex flex-wrap gap-3">
                            <span className="rounded-full border border-neutral-700 px-3 py-1 text-xs text-neutral-300">{tOld("EU shipping available", "Pristatymas ES")}</span>
                            <span className="rounded-full border border-neutral-700 px-3 py-1 text-xs text-neutral-300">{tOld("Ask for quote", "Klauskite kainos")}</span>
                            <span className="rounded-full border border-neutral-700 px-3 py-1 text-xs text-neutral-300">LT / EN </span>
                        </div>
                        <div className="mt-8 flex flex-col sm:flex-row gap-3 sm:gap-4">
                                                <button onClick={() =>
                                                    scrollTo("catalog")} className="rounded-2xl bg-yellow-500 text-black px-4 py-2 font-medium hover:bg-yellow-400">{tOld("Browse products", "Žiūrėti produktus")}</button>
                                                <button onClick={() =>
                                                    scrollTo("contact")} className="rounded-2xl border border-neutral-700 px-4 py-2 text-white hover:bg-neutral-900">{tOld("Send inquiry", "Siųsti užklausą")}</button>
                        </div>
                    </div>
            ...
            <section id="catalog" className="border-b border-neutral-800">
                <div className="mx-auto max-w-7xl px-4 py-10">
                    <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                        <div>
                            <h2 className="text-2xl font-bold">{tOld("Products", "Produktai")}</h2>
                            <p className="text-neutral-400">{tOld("Search or filter, then send an inquiry.", "Ieškokite arba filtruokite ir siųskite užklausą.")}</p>
                        </div>
                        <div className="grid grid-cols-1 gap-3 md:grid-cols-3 md:items-center">
                            <input value={q} onChange={e => setQ(e.target.value)} placeholder={tOld("Search by name or SKU", "Paieška pagal pavadinimą ar SKU")} className="w-full rounded-xl border border-neutral-700 bg-neutral-900 text-white placeholder:text-neutral-500 px-3 py-2 text-sm" />
                            <select value={cat} onChange={e => setCat(e.target.value)} className="w-full rounded-xl border border-neutral-700 bg-neutral-900 text-white px-3 py-2 text-sm">
                                <option className="bg-neutral-900" value="">{tOld("All categories", "Visos kategorijos")}</option>
                                {CATEGORIES.map(c => (
                                    <option className="bg-neutral-900" key={c.id} value={c.id}>{catLabel(c)}
                                                ...
                                                <button onClick={() =>
                                                    scrollTo("contact")} className="rounded-xl border border-neutral-700 px-3 py-2 text-center text-sm bg-neutral-900 hover:bg-neutral-800">{tOld("General inquiry", "Bendra užklausa")}</button>
                        </div>
                    </div>
                    ...
                    <div className="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                            ...
                            className="rounded-3xl border border-neutral-800 bg-neutral-900 p-5 hover:shadow-yellow-500/10 hover:shadow transition">
                                ...
                                <div className="mt-4 flex items-center justify-between gap-3">
                                    ...
                                    <span className="rounded-2xl border border-neutral-700 px-3 py-2 text-sm">{tOld("Open", "Atidaryti")}</span>
                                </div>
                            </Link>
                        ))}
                    </div>
            ...
            {/* Contact */}
                ...
                <div className="mx-auto max-w-7xl px-4 py-12 grid gap-8 md:grid-cols-2">
                    <div>
                        <h2 className="text-2xl font-bold">{tOld("Contact sales", "Susisiekite su pardavėju")}</h2>
                        <p className="text-neutral-400 mt-2">{tOld("Send your machine model and which attachment You're interested in.", "Parašykite technikos modelį ir kuris priedas Jus domina.")}</p>
                            <ul className="mt-4 text-sm text-neutral-300 space-y-1">
                                ...
                                <li className="mt-8 mb-3 text-lg font-semibold text-neutral-200 tracking-wide">
                                    {tOld("Follow us on social media:", "Sekite mus socialiniuose tinkluose:")}
                                </li>
                                    {/* Social Icons */}
                        ...
                        <form onSubmit={handleSubmit} className="rounded-3xl border border-neutral-800 bg-neutral-900 p-5 grid gap-3">
                            ...
                            <input name="name" required placeholder={tOld("Your name", "Jūsų vardas")} className="rounded-xl border border-neutral-700 bg-black text-white placeholder:text-neutral-500 px-3 py-2 text-sm" />
                            ...
                            <input name="company" placeholder={tOld("Company (optional)", "Įmonė (neprivaloma)")} className="rounded-xl border border-neutral-700 bg-black text-white placeholder:text-neutral-500 px-3 py-2 text-sm" />
                            <textarea name="message" rows={5} defaultValue={enquire ? `${tOld("Interested in:", "Domina:")} ${productTitle(enquire) } (SKU: ${enquire.id})
` : ""} placeholder={tOld("Message (product, machine model, questions)", "Žinutė (produktas, technikos modelis, klausimai)")} className="rounded-xl border border-neutral-700 bg-black text-white placeholder:text-neutral-500 px-3 py-2 text-sm" />
                            <button
                                type="submit"
                                disabled={sending}
                            ...
                            >
                                {sending ? (
                                    ...
                                    tOld("Send inquiry", "Siųsti užklausą")
                                )}
                            </button>
            ...
            {enquire && (
                <div className="fixed inset-0 z-50 bg-black/70 grid place-items-center p-4" onClick={() => setEnquire(null)}>
                        ...
                        <h3 className="text-xl font-bold">{tOld("Send inquiry", "Siųsti užklausą")}</h3>
                        <p className="text-sm text-neutral-300 mt-1">{tOld("Product:", "Produktas:")} {enquire.title} (SKU: {enquire.id})</p>
                            <form onSubmit={handleSubmit} className="mt-4 grid gap-3">
                                ...
                                <input name="name" required placeholder={tOld("Your name", "Jūsų vardas")} className="rounded-xl border border-neutral-700 bg-black text-white placeholder:text-neutral-500 px-3 py-2 text-sm" />
                                <input name="email" required type="email" placeholder="Email" className="rounded-xl border border-neutral-700 bg-black text-white placeholder:text-neutral-500 px-3 py-2 text-sm" />
                                <textarea name="message" rows={5} defaultValue={`${tOld("Interested in:", "Domina:")} ${productTitle(enquire) } (SKU: ${enquire.id})
`} placeholder={t("Message", "Žinutė")} className="rounded-xl border border-neutral-700 bg-black text-white placeholder:text-neutral-500 px-3 py-2 text-sm" />
                            <div className="flex items-center justify-end gap-2">
                                <button type="button" onClick={() => setEnquire(null)} className="rounded-2xl border border-neutral-700 px-4 py-2 hover:bg-neutral-900">{tOld("Cancel", "Atšaukti")}</button>
                                    <button
                                        type="submit"
                                        disabled={sending}
                                        className={`rounded-2xl px-4 py-2 font-medium transition ${sending
